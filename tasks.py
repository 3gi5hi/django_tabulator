...
def coverage(c):
    ...
    """
    c.run("coverage run --source django_tabulator runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django_tabulator.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_tabulator")
...
def lint(c):
    ...
    """
    c.run("flake8 django_tabulator tests")
...
def release(c, bumpsize=''):
    ...
    import django_tabulator
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_tabulator.__version__))
    c.run("git push --tags")
